...
export function HydrationTrackerSection({ hydrationData, onAddWater, isLogging, logError }: HydrationTrackerSectionProps) {
  ...
  const handleAddWater = async (amount: number) => {
    setJustAdded(false);
    const success = await onAddWater(amount);
    if (success) {
      setJustAdded(true);
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1"
            />
            <Button onClick={handleAddCustom} disabled={!customAmount || isLogging}>
              <Plus className="h-4 w-4 mr-2" />
              {isLogging ? 'Adding…' : 'Add Water'}
            </Button>
          </div>
          {logError && (
            <p className="text-xs text-destructive mt-1">{logError}</p>
          )}
        </div>
