...
export function LoginForm() {
  ...
  const simulateLogin = (): Promise<{ success: boolean; role?: UserRole; onboardingStatus?: OnboardingStatus; message?: string }> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        if (formData.email === 'user@example.com' && formData.password === 'password123') {
          resolve({ success: true, role: 'user', onboardingStatus: 'complete' });
        } else if (formData.email === 'newuser@example.com' && formData.password === 'password123') {
          resolve({ success: true, role: 'user', onboardingStatus: 'incomplete' });
        } else if (formData.email === 'admin@mazaj.com' && formData.password === 'admin123') {
          resolve({ success: true, role: 'admin', onboardingStatus: 'complete' });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      } else if (result.onboardingStatus === 'incomplete') {
        setRedirectMessage('Completing your profile setup...');
        setTimeout(() => {
          navigate('/onboarding');
        }, 1500);
      } else {
      ...
      setCredentialsError(result.message || 'Login failed');
